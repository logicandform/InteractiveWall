...
class InfoCollectionItemView: NSCollectionViewItem {
    ...
    static func height(for record: Record, width: CGFloat) -> CGFloat {
        let dateText = NSAttributedString(string: record.date ?? "", attributes: style.recordDateAttributes)
    ...
    private func load(_ record: Record?) {
        ...
        dateTextField.attributedStringValue = NSAttributedString(string: record.date ?? "", attributes: dateAttributes)
